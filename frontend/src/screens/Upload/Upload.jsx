...
function DatasetCard({ dataset, file, isDragOver, onDrop, onDragOver, onDragLeave, onFileChange }) {
  ...
  // Detect the moment a card *becomes* successful (rather than just being
  // successful) so the sweep + tick animation only plays on that transition
  // — not on every re-render while it stays "ok". Bumping successPulse
  // gives the overlay/tick a fresh React key, which replays the CSS
  // animation each time a file is (re)validated successfully.
  const prevStatusRef = useRef(status);
  const [successPulse, setSuccessPulse] = useState(0);

  useEffect(() => {
    if (status === "ok" && prevStatusRef.current !== "ok") {
  ...
  return (
    ...
    >
      ...
      {/* Success wash: a solid green panel that scales up from the bottom
          edge, sweeping the card green from bottom to top. Sits behind the
          rest of the card content (z-10) but above the plain white
          background (z-0 by default). */}
      {isOk && (
        <div
      ...
      {/* Filename: positioned against the OUTER card (not the inner content
          wrapper above), so it drops into the empty space every card
          already has at the bottom from being stretched to match the
          tallest card in the row — instead of adding its own height or
          crowding right under the button. */}
      {file?.name && (
      ...
      {/* Tick badge: scales up from 0 once the sweep has mostly finished. */}
      {isOk && (
        <div
 ...
 * }} props
 */
export default function UploadStep({ files, setFiles, completedKeys, onContinue, onNavigate }) {
  ...
  return (
    <PageShell
      ...
      topbarIcon={UploadCloud}
      topbarLabel="Upload / Select Railway Data"
    >
      <HeroBanner
        title="Upload / Select Railway Data"
        subtitle="Upload the datasets required to begin intelligent block planning."
      />
